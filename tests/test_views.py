...
class BaseTestCase(unittest.TestCase):
    ...
    def signup_admin(self, user_name="admin", email="administrator@gmail.com", password="admin123", role="admin"):

        """
        Method for registering an admin with dummy data
        """
        return self.test_client.post(
            'api/v1/auth/signup',
            data=json.dumps(dict(
    ...
    #     with self.test_client:
    #         response = self.signup_admin("admin", "administrator@gmail.com", "admin123", "admin")
    #         data = json.loads(response.data.decode())
    #         self.assertEqual(data['message'], 'Thank you for signing up')
    #         self.assertEqual(data['status'], 201)
    #         self.assertEqual(response.status_code, 200)
    ...
    def test_get_a_delivery_order(self):
        with self.test_client:
            """Tests api to get a specific order without token"""
            response = self.test_client.get('/api/v1/orders/1',
                                             json=self.test_create_order)
            data = json.loads(response.data.decode())
            self.assertEqual(data['status'], 401)
    ...
    def test_cancel_an_order_by_a_user(self):
        ...
        with self.test_client:
            ...
            # self.assertEqual(data['status'], 200)
            # self.assertEqual(response.status_code, 200)
            """ Test cancel non-existent order or user with token"""
    ...
    #     """Test test_cancel_all_orders_by_user without token"""
    #     with self.test_client:
    #         response = self.cancel_an_order("cancelled")
    #         data = json.loads(response.data.decode())
    #         self.assertEqual(data['message'], "Provide Token")
    ...
    #         self.assertEqual(data['status'], 200)
    #         self.assertEqual(response.status_code, 200)
    #         """ Test cancel non-existent user or order with token"""
    #         response = self.test_client.put('/api/v1/orders/9/cancel',
    #                                         json={'status': 'cancelled'},
    #                                          headers=dict(Authorization='Bearer ' + self.get_user_token()))
    #         data = json.loads(response.data.decode())
